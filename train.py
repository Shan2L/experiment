...
if __name__ == "__main__":
    ...
    train_dataset = LandslideDataset(train=True, labeled='labeled', transform=train_transform)
    val_dataset = LandslideDataset(train=False, transform=train_transform)
    ...
    if True:
        ...
        gen             = DataLoader(train_dataset, shuffle = shuffle, batch_size = batch_size, num_workers = 0, pin_memory=False,
                                    drop_last = False)
        gen_val         = DataLoader(val_dataset  , shuffle = shuffle, batch_size = batch_size, num_workers = 0, pin_memory=False, 
                                    drop_last = False)
        ...
        import json
        with open('./dataset/split/test.json', 'r')as valf:
            val_data = json.load(valf)
        val_lines = []
        for item in val_data:
            val_lines.append(item['img'])
        
        if local_rank == 0:
            eval_callback   = EvalCallback(net=model, input_shape=input_shape, num_classes=num_classes, image_ids=val_lines, dataset_path='./data/visual/mask', log_dir=log_dir, cuda=Cuda, \
        ...
        for epoch in range(Init_Epoch, UnFreeze_Epoch):
            ...
            fit_one_epoch(model_train, model, loss_history, eval_callback, optimizer, epoch, 
                    epoch_step, epoch_step_val, gen, gen_val, UnFreeze_Epoch+Freeze_Epoch, Cuda, dice_loss, focal_loss, cls_weights, num_classes, fp16, scaler, save_period, save_dir, local_rank)
