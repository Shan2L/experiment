...
def compute_mIoU(gt_dir, pred_dir, png_name_list, num_classes, name_classes=None):  
    ...
    mask_name_list = [x.replace('image', 'mask') for x in png_name_list]
    
    gt_imgs     = [join(gt_dir, x) for x in mask_name_list]  
    ...
    for ind in range(len(gt_imgs)): 
        ...
        #------------------------------------------------#
        pred = np.uint8(np.array(Image.open(pred_imgs[ind]))//255) 
        #------------------------------------------------#
